...
function clearData(data) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      aData = aData.split(
        new RegExp('.\\s*\\t*application/json\\s*\\t*.', 'i')
      );
    ...
    try {
      for (let idx = 0; idx < data.length; ++idx) {
        ...
        if (
          isStr1 > 0 ||
          isStr2 > 0 ||
          (stackComment1 == 0 && stackComment2 == 0)
        ) {
          strToReturn += c;
      ...
      for (let idx = 0; idx < methods.length; ++idx) {
        ...
        if (idx == methods.length - 1) {
          ...
          for (let idxPtn = 1; idxPtn < auxData.length; ++idxPtn) {
            let auxBytePosition = auxData[idxPtn]
              .split(']_])(')[1]
              .split('([_[')[0].length;
  ...
  try {
    for (let idx = 0; idx < response.length; ++idx) {
      ...
      if (
        res &&
        elem &&
        elem.replaceAll(' ', '').includes(res + '.setHeader(')
      ) {
...
async function getCallbackParameters(data) {
  ...
  try {
    ...
    for (let idx = 0; idx < splitedParams.length; ++idx) {
      ...
      let arrowFunctionPos = pos.split(
        new RegExp(`(\\s*\\t*=>\\s*\\n*\\t*\\{)`)
      );
      ...
      if (arrowFunctionPos.length == 1) {
        arrowFunctionWithoutCurlyBracketPos = pos.split(
          new RegExp(`(\\s*\\t*=>)`)
        );
      ...
      if (
        ...
        if (params[0] && params[0].includes(':')) {
          ...
          if (typeParam.includes('res')) {
            if (
              param.split(new RegExp(regex)).length === 1 &&
              param &&
              param.trim() !== ''
            ) {
              res.add(param);
...
async function functionRecognizerInData(data, functionName) {
  ...
  try {
    ...
    if (
      data.split(
        new RegExp(
          `\\w+${functionName}|${functionName}\\w+|\\w+${functionName}\\w+`
        )
      ).length > 1
    ) {
      ...
      if (data.length > 1) {
        for (let idxHeader = 1; idxHeader < data.length; ++idxHeader) {
          let startComment = utils.getFirstPosition(
            '/*',
            data[idxHeader].split('//')[0]
          );
          let endComment = utils.getFirstPosition(
            '*/',
            data[idxHeader].split('//')[0]
          );
...
async function popFunction(data) {
  ...
  try {
    ...
    if (isArrowFunction || isTraditionalFunction) {
      ...
      func = func.join('{');
      func =
        signatureFunc + (await utils.stackSymbolRecognizer(func, '{', '}'));
      return func.trim();
