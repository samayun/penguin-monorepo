  ...
  try {
    ...
    for (let idx = 0; idx < variablesNode.length; ++idx) {
      ...
      if (
        ...
        let value = resolveVariableValue(
          item.declarations[0].init,
          jsObj.variables
        );
        jsObj.variables.push({
          name,
          end,
          kind: item.kind,
          typeof: typeof value,
...
async function jsParserEsModule(data, onlyPrimitiveTypes = true) {
  try {
    let dataAux = data;
    dataAux =
      dataAux && dataAux.split(new RegExp('(const|let|var)(\\s+.*\\s*=\\s*)'));
    if (dataAux.length > 3) {
      dataAux[0] = '/*' + dataAux[0] + '*/';
      for (let idx = 3; idx < dataAux.length; idx += 3) {
        ...
        if (varValue.split(/^null/).length > 1) {
          ...
          // object or array
          let symbol = varValue[0];
          let dat = await utils.stack0SymbolRecognizer(
            varValue,
            symbol,
            null,
            true
 ...
 * @returns
 */
async function resolvePathVariables(
  rawPath,
  bytePosition,
  jsParsed,
  importedFiles
    ...
    if (
      ...
      for (let index = 0; index < auxPath.length; ++index) {
        ...
        if (resolvedVariables && resolvedVariables.length > 0) {
          ...
          let value = null;
          if (
            varName.split('.').length > 1 &&
            resolvedVariables[0].typeof == 'object'
          ) {
