      ...
      Object.entries(def).forEach((elem) => {
        if (typeof elem[1] === 'object') {
          ...
          if (resp.type == 'array') {
            ...
            resp.properties[elem[0]] = formatDefinitions(
              elem[1],
              resp,
              constainXML
            );
...
function getPath(elem, autoMode) {
  ...
  try {
    ...
    if (autoMode && !elem.includes(statics.SWAGGER_TAG + '.path')) {
      ...
      if (
        (quotMark == '"' || quotMark == "'" || quotMark == '`') &&
        line.split(quotMark).length > 2
      ) {
...
function getMethodTag(data, reference) {
  try {
    if (data.includes(statics.SWAGGER_TAG + '.method')) {
      let method = data.split(
        new RegExp(statics.SWAGGER_TAG + '.method' + '\\s*\\=\\s*')
      )[1];
...
function getForcedEndpoints(aData, reference) {
  try {
    let aForcedsEndpoints = aData.split(
      new RegExp(
        `.*${statics.SWAGGER_TAG}.start.*|.*${statics.SWAGGER_TAG}.end.*`,
        'i'
      )
    );
    if (aForcedsEndpoints.length > 1) {
      ...
      aForcedsEndpoints = aForcedsEndpoints.map((e) => {
        ...
        return (e =
          '[_[' +
          method +
          "]_])('/_undefined_path_0x" +
          e.length.toString(16) +
          "', " +
          e);
      });
    } else {
...
function getAutoTag(data) {
  try {
    if (data.includes(statics.SWAGGER_TAG + '.auto')) {
      let auto = data.split(
        new RegExp(statics.SWAGGER_TAG + '.auto' + '\\s*\\=\\s*')
      )[1];
...
async function getParametersTag(data, objParameters, reference) {
  ...
  try {
    ...
    let swaggerParameters = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.parameters`)
    );
    swaggerParameters.shift();
    for (let idx = 0; idx < swaggerParameters.length; ++idx) {
      let parameter = await utils.stack0SymbolRecognizer(
        swaggerParameters[idx],
        '{',
        '}'
      );
      let name = swaggerParameters[idx]
        .split(new RegExp('\\[|\\]'))[1]
        .replaceAll("'", '');
       ...
       */
      if (
        objParameters[name].in &&
        objParameters[name].in.toLowerCase() === 'path' &&
        !objParameters[name].required
      ) {
      ...
      if (
        !objParameters[name].type &&
        !objParameters[name].schema &&
        objParameters[name].in != 'body'
      ) {
      ...
      if (
        objParameters[name].type &&
        objParameters[name].in &&
        objParameters[name].in.toLowerCase() === 'body'
      ) {
      ...
      if (
        objParameters[name].required &&
        typeof objParameters[name].required === 'string'
      ) {
      ...
      if (objParameters[name] && objParameters[name]['@schema']) {
        ...
        if (objParameters[name].schema['@enum']) {
          ...
          objParameters[name].schema = formatDefinitions(
            objParameters[name].schema
          );
        }
      }
...
async function getRequestBodyTag(data, reference) {
  try {
    ...
    data = data
      ...
      .replaceAll('`', "'")
      .replaceAll('\n', ' ');
    let swaggerRequestBody = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.requestBody`)
    );
    swaggerRequestBody.shift();
    for (let idx = 0; idx < swaggerRequestBody.length; ++idx) {
      let parameter = await utils.stack0SymbolRecognizer(
        swaggerRequestBody[idx],
        '{',
        '}'
      );
      if (parameter) {
...
async function getProducesTag(data, reference) {
  try {
    ...
    let produces = [];
    let swaggerProduces = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.produces\\s*\\=\\s*`)
    );
    swaggerProduces.shift();
    for (let idx = 0; idx < swaggerProduces.length; ++idx) {
      let prod = await utils.stack0SymbolRecognizer(
        swaggerProduces[idx],
        '[',
        ']'
      );
...
async function getConsumesTag(data, reference) {
  try {
    data = data.replaceAll('\n', ' ').replaceAll('__¬¬¬__', '"');
    let consumes = [];
    let swaggerConsumes = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.consumes\\s*\\=\\s*`)
    );
    swaggerConsumes.shift();
    for (let idx = 0; idx < swaggerConsumes.length; ++idx) {
      let cons = await utils.stack0SymbolRecognizer(
        swaggerConsumes[idx],
        '[',
        ']'
      );
...
async function getResponsesTag(data, objResponses, reference) {
  ...
  try {
    data = data
      .replaceAll('\n', ' ')
      .replaceAll('#definitions', '#/definitions');
    let swaggerResponses = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.responses`)
    );
    swaggerResponses.shift();
    for (let idx = 0; idx < swaggerResponses.length; ++idx) {
      let statusCode = swaggerResponses[idx]
        .split(new RegExp('\\[|\\]'))[1]
        .replaceAll('"', '')
        .replaceAll("'", '')
        .replaceAll('`', '');
      ...
      if (
        ...
        // has object
        let objResp = await utils.stack0SymbolRecognizer(
          swaggerResponses[idx],
          '{',
          '}'
        );
      ...
      if (
        getOpenAPI() &&
        objResponses[statusCode] &&
        objResponses[statusCode].schema &&
        !objResponses[statusCode].content
      ) {
        objResponses[statusCode] = {
          ...objResponses[statusCode],
          content: {
...
function getDescription(data, reference) {
  try {
    ...
    if (
      (quotMark == '"' || quotMark == "'" || quotMark == '`') &&
      swaggerDescription.split(quotMark).length > 2
    ) {
...
function getSummary(data, reference) {
  try {
    let swaggerSummary = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.summary\\s*\\=\\s*`)
    )[1];
    const quotMark = swaggerSummary[0];
    if (
      (quotMark == '"' || quotMark == "'" || quotMark == '`') &&
      swaggerSummary.split(quotMark).length > 2
    ) {
...
function getOperationId(data, reference) {
  try {
    ...
    if (
      (quotMark == '"' || quotMark == "'" || quotMark == '`') &&
      swaggerOperationId.split(quotMark).length > 2
    ) {
...
function getTags(data, reference) {
  try {
    let tags = [];
    let swaggerTags = data.split(
      new RegExp(`${statics.SWAGGER_TAG}.tags\\s*\\=\\s*`)
    )[1];
...
async function getSecurityTag(data, reference) {
  try {
    ...
    let securityParameters = await utils.stack0SymbolRecognizer(
      swaggerSecurity,
      '[',
      ']'
    );
