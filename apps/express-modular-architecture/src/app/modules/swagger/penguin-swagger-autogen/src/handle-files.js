...
function readEndpointFile(
  ...
  return new Promise((resolve) => {
    ...
    fs.readFile(filePath, 'utf8', async function (err, data) {
       ...
       */
      let jsParsed = codeParser.jsParser(
        await handleData.removeComments(data, true)
      );
      ...
      dataToGetPatterns = await handleData.removeComments(
        dataToGetPatterns,
        false
      );
      dataToGetPatterns = await handleData.removeStrings(dataToGetPatterns);
      dataToGetPatterns = await handleData.removeInsideParentheses(
        dataToGetPatterns,
        true
      );
      ...
      if (dataToGetPatterns) {
        let lastElem = dataToGetPatterns
          .split('.')
          .slice(-1)[0]
          .replaceAll(' ', '');
      ...
      if (regexNewRouter.test(aData)) {
        ...
        for (let index = 1; index < routes.length; index += 2) {
          ...
          if (route.includes('Router') && prop.includes('prefix')) {
            ...
            if (
              prefix &&
              prefix.split(new RegExp('\\s*prefix\\s*\\n*\\t*\\:').length > 1)
            ) {
              prefix = prefix
      ...
      if (regex != '') {
        ...
        aRoutes = aDataRawCleaned.split(
          new RegExp(`\\s*\\t*\\w\\s*\\t*\\.\\s*\\t*use\\s*\\t*\\(`)
        );
        if (aRoutes.length > 1) {
          aRoutes.shift();
        }
        ...
        for (let idxElem = 0; idxElem < aData.length; idxElem++) {
          ...
          if (
            rawPath &&
            rawPath.split(new RegExp('\\s*function\\s*\\(')).length > 1
          ) {
            rawPath = '';
          }
          ...
          if (elem && elem.includes('[_[') && elem.includes(']_]')) {
            ...
            let prefixFound = propRoutes.find(
              (r) => r.routeName === predefPattern
            );
            if (prefixFound) {
          ...
          if (isChained) {
            ...
            const found = localRouteMiddlewares.find(
              (midd) =>
                midd.rawRoute &&
                midd.rawRoute.split(new RegExp(endpointRegex)).length > 1
            );
            if (found) {
          ...
          if (
            (elemParam && elemParam.split(',').length > 1 && !forced) ||
            predefMethod === 'use'
          ) {
            ...
            if (auxElem.split(',').length > 1 || predefMethod === 'use') {
              ...
              functionsStr = functionsStr.split(
              ...
              for (let idxFunc = 0; idxFunc < 15; ++idxFunc) {
                ...
                if (
                  ...
                  let params = functionsStr
                    .trim()
                    .split(new RegExp('\\s*\\t*=>\\s*\\n*\\t*'));
                  if (
                    params &&
                    params.length > 1 &&
                    params[0].trim().slice(-1)[0] !== ')'
                  ) {
                    let paramsAux = params[0].split(
                      new RegExp('\\s+|\\n+|\\t+|\\,|\\.|\\;|\\:')
                    );
                    paramsAux = paramsAux.slice(-1)[0];
                    if (
                      paramsAux.split(/\*|\\|\/|\(|\)|\{|\}|\[|\]/).length ===
                        1 &&
                      paramsAux !== ''
                    )
                ...
                let funcNotReferenced = await handleData.popFunction(
                  functionsStr
                );
                if (predefMethod == 'use' && funcNotReferenced) {
                  ...
                  if (funcNotReferenced.trim()[0] == '(') {
                    ...
                    if (funcParams) {
                      ...
                      if (funcNotRefFormated) {
                        refFunc = funcNotRefFormated.split(
                  ...
                  if (
                    functionsStr &&
                    functionsStr.split(funcNotReferenced).length > 1
                  ) {
                    functionsStr = functionsStr.replace(funcNotReferenced, ' ');
                  } else {
                    let params = await utils.stack0SymbolRecognizer(
                      funcNotReferenced,
                      '(',
                      ')'
                    );
                    if (
                      params &&
                      functionsStr.split('(' + params + ')').length > 1
                    ) {
                      functionsStr = functionsStr.replace(
                        '(' + params + ')',
                        ' '
              ...
              endpointSwaggers = await handleData.getSwaggerComments(
                functionsStr
              );
            ...
            for (let index = 0; index < functions.length; index++) {
              ...
              if (rexRequire.test(func)) {
                ...
                if (exportPath && exportPath.includes('./')) {
                  if (exportPath.includes('../')) {
                    ...
                    RelativePathBacked = RelativePathBacked.slice(
                      0,
                      -1 * foldersToBack
                    );
                ...
                if (func.includes('(') && func.includes(')')) {
                  let params = await utils.stack0SymbolRecognizer(
                    ...
                    '(',
                    ')'
                  ); // TODO: get array with all strings and try to find with each one
                  if (
                    params &&
                ...
                idx = importedFiles.findIndex(
                  (e) =>
                    e.varFileName && varFileName && e.varFileName == varFileName
                );
                if (idx == -1) {
              ...
              if (idx > -1 || exportPath) {
                ...
                if (
                  ...
                  let dataIndexFile = await utils.getFileContent(
                    pathFile + extension
                  );
                  if (dataIndexFile) {
                    pathFile = pathFile.split('/').slice(0, -1).join('/'); // removing '/index'
                    let importsIndexFile = await getImportedFiles(
                      dataIndexFile,
                      pathFile
                    );
                ...
                if (!refFunction) {
                  if (!functionName) {
                    ...
                    if (dataIndexFile) {
                      ...
                      let exportsIndexFile = await getImportedFiles(
                        dataIndexFile,
                        pathFile
                      );
                      let idx = -1;
                      ...
                      if (idx == -1) {
                        exportsIndexFile.forEach((imp) => {
                          ...
                          let found =
                            imp && imp.exports
                              ? imp.exports.find(
                                  (e) => e.varAlias && e.varAlias == 'default'
                                )
                              : null;
                          if (found) {
                 ...
                 */
                let refFunction = await handleData.functionRecognizerInData(
                  aDataRaw,
                  varFileName
                );
          ...
          for (let efIdx = 0; efIdx < endpointFunctions.length; ++efIdx) {
            ...
            const callbackParameters = await handleData.getCallbackParameters(
              ',' + ef.func
            );
          ...
          if (endpointFunctions && endpointFunctions.length == 0) {
            ...
            for (let _idxEF = 0; _idxEF < endpointFunctions.length; ++_idxEF) {
              ...
              if (autoMode && !req && !res) {
                if (forced) {
                  ...
                  const callbackParameters =
                    endpointFunctions[_idxEF].callbackParameters;
              ...
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.operationId')
              ) {
                objEndpoint[path][method]['operationId'] =
                  swaggerTags.getOperationId(endpoint, reference);
              }
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.summary')
              ) {
                objEndpoint[path][method]['summary'] = swaggerTags.getSummary(
                  endpoint,
                  reference
                );
              }
              ...
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.requestBody')
              ) {
                objEndpoint[path][method].requestBody =
                  await swaggerTags.getRequestBodyTag(endpoint, reference);
              }
              if (!swaggerTags.getOpenAPI()) {
                if (
                  endpoint &&
                  endpoint.includes(statics.SWAGGER_TAG + '.produces')
                ) {
                  objEndpoint[path][method].produces =
                    await swaggerTags.getProducesTag(endpoint, reference);
                }
                if (
                  endpoint &&
                  endpoint.includes(statics.SWAGGER_TAG + '.consumes')
                ) {
                  objEndpoint[path][method].consumes =
                    await swaggerTags.getConsumesTag(endpoint, reference);
                }
              }
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.responses')
              ) {
                objResponses = await swaggerTags.getResponsesTag(
                  endpoint,
                  objResponses,
                  reference
                );
              ...
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.description')
              ) {
                objEndpoint[path][method]['description'] =
                  swaggerTags.getDescription(endpoint, reference);
              }
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.tags')
              ) {
                objEndpoint[path][method]['tags'] = swaggerTags.getTags(
                  endpoint,
                  reference
                );
              }
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.security')
              ) {
                objEndpoint[path][method]['security'] =
                  await swaggerTags.getSecurityTag(endpoint, reference);
              }
              if (
                endpoint &&
                endpoint.includes(statics.SWAGGER_TAG + '.deprecated')
              ) {
                objEndpoint[path][method]['deprecated'] =
                  swaggerTags.getDeprecatedTag(endpoint, reference);
              }
              ...
              if (
                autoMode &&
                endpoint &&
                ((req && req.length > 0) || (res && res.length > 0))
              ) {
                ...
                if (req) {
                  objParameters = handleData.getHeaderQueryBody(
                    endpoint,
                    req,
                    objParameters
                  ); // Search for parameters in the query and body
                  if (method === 'get' && objParameters['__obj__in__body__']) {
                    delete objParameters['__obj__in__body__'];
                  }
                  objParameters = handleData.getQueryIndirectly(
                    endpoint,
                    req,
                    objParameters
                  ); // Search for parameters in the query (indirectly)
                ...
                if (res) {
                  objResponses = handleData.getStatus(
                    endpoint,
                    res,
                    objResponses
                  ); // Search for response status
                  objEndpoint = handleData.getHeader(
                    endpoint,
                    path,
                    method,
                    res,
                    objEndpoint
                  ); // Search for resonse header
                }
              }
              ...
              if (
                ...
                if (
                  objEndpoint[path][method].tags &&
                  objEndpoint[path][method].tags.length == 0
                ) {
                ...
                if (
                  objEndpoint[path][method].parameters &&
                  objEndpoint[path][method].parameters.length > 0 &&
                  objEndpoint[path][method].parameters.find(
                    (e) => e.in === 'body'
                  )
                ) {
                  let body = objEndpoint[path][method].parameters.find(
                    (e) => e.in === 'body'
                  );
                  if (
                    objInBody &&
                  ...
                  if (
                    ...
                    !body.schema &&
                    (!body.type ||
                      (body.type && body.type.toLowerCase() == 'object'))
                  ) {
              ...
              if (parameters && parameters.length > 0) {
                objEndpoint[path][method].parameters = parameters.map((p) => {
                  if (
                    p.schema &&
                    p.schema.properties &&
                    p.schema.properties['__AUTO_GENERATE__']
                  ) {
                    p.schema.properties = {};
                  }
                  return p;
      ...
      if (aRoutes && aRoutes.length >= 1) {
        for (let file = 0; file < aRoutes.length; file++) {
          ...
          if (rexRequire.test(data)) {
            if (
              data &&
              data.split(new RegExp('\\(\\s*__dirname\\s*\\+\\s*\\"?\\\'?\\`?'))
                .length > 1
            ) {
            ...
            if (exportPath && exportPath.includes('./')) {
              ...
              const isDirectory =
                ...
                fs.lstatSync(exportPath).isDirectory()
                  ? true
                  : false;
              if (isDirectory) {
          ...
          if (
            ...
            if (found) {
              ...
              let content = await utils.getFileContent(
                found.fileName + extension
              );
              if (content) {
                // Try to find the variable
                content = await handleData.removeComments(content);
                let jsParsedContent = await codeParser.jsParserEsModule(
                  content
                );
          ...
          if (data.split(',').length == 1) {
            ...
            if (
              data.trim()[0] === ',' &&
              (await utils.stack0SymbolRecognizer(data, '[', ']'))
            ) {
              /**
               * Multiple express Routers under the same name space
               * Issue: 61
               */
              let auxOfFileName = await utils.stack0SymbolRecognizer(
                data,
                '[',
                ']'
              );
            ...
            for (
              let idxFileName = 0;
              idxFileName < listOfFileName.length;
              ++idxFileName
            ) {
              /**
               * CASE:
               ...
               */
              if (
                listOfFileName[idxFileName].split(new RegExp('new\\s+'))
                  .length > 1
              ) {
                if (listOfFileName[idxFileName].slice(-1)[0] == ')') {
                  listOfFileName[idxFileName] = listOfFileName[
                    idxFileName
                  ].slice(0, -1);
                }
                ...
                if (
                  listOfFileName[idxFileName].split(
                    new RegExp('\\([\\s|\\S]*\\)')
                  ).length > 1
                ) {
                  listOfFileName[idxFileName] = listOfFileName[
                  ...
                  listOfFileName[idxFileName] =
                    listOfFileName[idxFileName].join('');
                }
                if (listOfFileName[idxFileName].includes('.')) {
                  refFunc = listOfFileName[idxFileName].split('.')[1];
                  listOfFileName[idxFileName] =
                    listOfFileName[idxFileName].split('.')[0];
                }
              }
              /* END CASE */
          ...
          for (
            ...
            if (
              ...
              if (found) {
                let extension = await utils.getExtension(found.fileName);
                const content = await utils.getFileContent(
                  found.fileName + extension
                );
                if (content) {
                  ...
                  let varRouteFound = content
                    .split(
                      new RegExp(
                        '(const|let|var)(\\s+\\w+\\s*\\=\\s*Router\\(\\))'
                      )
                    )
                    .find((e) => e.replaceAll(' ', '').includes('=Router()'));
                  if (
                    varRouteFound &&
                    varRouteFound.split('=')[0].trim() == functionName
                  ) {
            ...
            if (idx > -1 || exportPath) {
              ...
              if (
                ...
                if (resp && resp.isDirectory) {
                  let extension = await utils.getExtension(
                    obj.fileName + '/index'
                  );
...
async function getImportedFiles(data, relativePath) {
  ...
  try {
    ...
    let requireds = data
      .replaceAll('\n', ' ')
      .split(
        new RegExp(`\\s*\\t*const\\s+|\\s*\\t*var\\s+|\\s*\\t*let\\s+`, 'i')
      );
    ...
    if (importeds && importeds.length > 1) {
      ...
      let tsPaths = [];
      let tsconfig = await utils.getFileContent(
      ...
      // Verify if .eslintrc
      let eslintConfig = await utils.getFileContent(
      ...
      for (let index = 0; index < importeds.length; ++index) {
        ...
        fileName &&
        fileName.split(new RegExp(' from |\\}\\s*from\\s*\\"?\\\'?\\`?', 'i'))
          .length > 1
          ? (fileName = fileName
              .split(new RegExp(' from |\\}\\s*from\\s*\\"?\\\'?\\`?', 'i'))[1]
              .trim())
          : imp.split(new RegExp(' from |\\}\\s*from\\s*\\"?\\\'?\\`?', 'i'))
              .length > 1
          ? (fileName = imp
              .split(new RegExp(' from |\\}\\s*from\\s*\\"?\\\'?\\`?', 'i'))[1]
              .trim())
        ...
        let found = tsPaths.find(
          (p) => p[0] && p[0].split('/*')[0] == pathPattern
        );
        ...
        if (found) {
          ...
          if (Array.isArray(found[1])) {
            ...
            if (realPath) {
              ...
              fileName =
                './' +
                fileName.replace(new RegExp('^' + refFileName), realPath);
        ...
        if (fileName.includes('./')) {
          ...
          if (relativePath) {
            ...
            if (fileName.includes('../')) {
              ...
              pathFile =
                RelativePathBacked + '/' + fileName.replaceAll('../', ''); //.replaceAll('//', '/')
            } else {
          ...
          obj.fileName = pathFile;
          obj.isDirectory =
            fs.existsSync(pathFile) && fs.lstatSync(pathFile).isDirectory()
              ? true
              : false;
          ...
          if (obj.isDirectory && obj.exports.length > 0) {
            ...
            if (indexExtension != '') {
              // index exist
              let dataFile = await utils.getFileContent(
                pathFile + '/index' + indexExtension
              );
              if (dataFile) {
    ...
    if (requireds && requireds.length > 0) {
      for (let index = 0; index < requireds.length; ++index) {
        ...
        let varFileName = req
          .split(new RegExp(`=\\s*\\t*require\\s*\\t*\\(`, 'i'))[0]
          .trim();
        ...
        let fileName = req
          .split(new RegExp(`=\\s*\\t*require\\s*\\t*\\(`, 'i'))[1]
          .trim();
          ...
          if (fileName.split(new RegExp(`.json`, 'i')).length == 1) {
            ...
            if (relativePath) {
              ...
              if (fileName.includes('../')) {
                ...
                pathFile =
                  RelativePathBacked + '/' + fileName.replaceAll('../', '');
              } else {
                pathFile = relativePath + fileName.replaceAll('./', '/');
              }
            } else {
            ...
            // Checking if reference is to file
            if (obj.isDirectory) {
              let indexExtension = await utils.getExtension(
                pathFile + '/index'
              );
              if (indexExtension != '') {
                // index exist
                  ...
                  obj.exports.map((oExp) => {
                    if (
                      ...
                      if (addPath.includes('../')) {
                        ...
                        oExp.path =
                          relativePath + addPath.replaceAll('./', '/');
                      }
                    }
                  });
                }
    ...
    fs.readFile(filePath, 'utf8', async function (err, data) {
      ...
      try {
        ...
        cleanedData = cleanedData.split(
          new RegExp('\\s*=\\s*asyncHandler\\s*\\(')
        ); // TODO: Implement method to remove 'fooToRemove' in this case: const|let|var foo = fooToRemove(...). Issue: #56.
        ...
        if (
          cleanedData.split(new RegExp('\\s*\\n*\\t*=>\\s*\\n*\\t*').length > 1)
        ) {
          let params = cleanedData
            .trim()
            .split(new RegExp('\\s*\\n*\\t*=>\\s*\\n*\\t*'));
          ...
          if (
            ...
            if (exports.length > 1) {
              ...
              if (path) {
                ...
                funcStr = await functionRecognizerInFile(
                  path + extension,
                  functionName,
                  false
                );
